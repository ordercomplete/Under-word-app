...
class BackupManager:
    ...
    MAX_BACKUPS = 10  # Максимальна кількість backup_* папок у backup/

    def __init__(self, target_root: Path):
        self.target_root = target_root
        timestamp = datetime.now().strftime('%Y%m%d%H%M%S')  # Windows-сумісний час
    ...
    def _prune_old_backups(self) -> None:
        ...
        root = self.target_root / "backup"
        if not root.exists():
            return
        folders = sorted(
            (p for p in root.iterdir()
             if p.is_dir() and p.name.startswith("backup_")),
            key=lambda p: p.name,  # ім'я backup_YYYYMMDDHHMMSS — сортується за часом
        )
...
class AgentLockManager:
    ...
    def get_all_target_paths(self) -> list:
        """Get all target paths of Agent files."""
        paths = []
        for target_path, info in self.lock_data.get("files", {}).items():
            if "targets" in info:
                paths.extend(info["targets"])
            else:
...
def cmd_gitignore(target_root: Path) -> int:
    """Add Agent paths to .gitignore."""
    lock = AgentLockManager(target_root)
    paths = lock.get_all_target_paths()
    if not paths:
        print("No installed Agent files found.")
        return 0
    _add_to_gitignore(target_root, paths)
    print(f"Added {len(paths)} paths to .gitignore")
    return 0
...
def _add_to_gitignore(target_root: Path, paths: list) -> None:
    ...
    new_paths = [p for p in paths if p not in existing_lines]

    if not new_paths:
        print("   All paths already in .gitignore")
        return

    section = "\n# === Comfy-smart-lady Agent (auto-managed) ===\n"
...
def _remove_from_gitignore(target_root: Path, paths: list) -> None:
    """Remove Agent paths from .gitignore."""
    gitignore = target_root / '.gitignore'
    ...
    for line in lines:
        stripped = line.strip()
        if stripped == "# === Comfy-smart-lady Agent (auto-managed) ===":
            skip_section = True
            continue
...
def main():
    ...
    parser.add_argument('--version', action='version', version='sync-agent.py 1.3.0')
    ...
    if not args.dry_run and args.apply:
        ...
        lock.save()
        _add_to_gitignore(target_root, lock.get_all_target_paths())
