...
# SKIP_DIRS: backup-chat/backup — локальні знімки стану, які НЕ розповсюджуються.
# backup-agent (zip-бекапи AGENT від watcher) навпаки ВХОДИТЬ до manifest root_files:
# ці файли живуть у репо та мають відображатися у структурі Агента.
SKIP_DIRS = {"__pycache__", "trash"}
