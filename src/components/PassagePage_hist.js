// src/components/PassagePage.js 11.02.2026 спроба створити історію переглядів з навігацією
import React, {
  useState,
  useEffect,
  useRef,
...
const Panel = memo(
  ({
    ...
    isNarrowScreen,
    initialRef, // ← новий
    initialVersions, // ← новий
    // ДОДАЄМО ЦІ ПРОПСИ
    passageHistoryState,
    onPassageBack,
    onPassageForward,
    setPassageHistoryState,
    isNavigatingFromHistory, // ← НОВИЙ ПРОПС
  }) => {
    const { get: getCache, set: setCache } = useChapterCache();
    // const [currentRef, setCurrentRef] = useState("GEN.1");
    // const [versions, setVersions] = useState([]);
    const [currentRef, setCurrentRef] = useState(initialRef || "GEN.1");
    const [versions, setVersions] = useState(initialVersions || []);
    const [chapterData, setChapterData] = useState({});
    const [loading, setLoading] = useState(false);
    const [translationsData, setTranslationsData] = useState(null);

    const [isInternalNavigation, setIsInternalNavigation] = useState(false); // ← НОВИЙ СТАН

    // Синхронізація з initialRef/initialVersions при навігації з історії
    // useEffect(() => {
    //   if (initialRef) {
    ...
    //   }
    // }, [initialRef, initialVersions]); // ← ВАЖЛИВО: залежності для синхронізації
    useEffect(() => {
      if (initialRef && isNavigatingFromHistory) {
        setIsInternalNavigation(true);
        setCurrentRef(initialRef);
      }
      if (initialVersions && isNavigatingFromHistory) {
        setIsInternalNavigation(true);
        setVersions(initialVersions);
      }
    }, [initialRef, initialVersions, isNavigatingFromHistory]);

    // Скидаємо флаг після оновлення
    useEffect(() => {
      if (isInternalNavigation) {
        setIsInternalNavigation(false);
      }
    }, [currentRef, versions, isInternalNavigation]);
    ...
    // Формування пар для InterlinearVerse оригінал
    const pairs = useMemo(() => {
    ...
    return (
      <div className="panel">
        <PassageOptionsGroup
          ...
          // ПЕРЕДАЄМО ПРОПСИ ДАЛІ
          passageHistoryState={passageHistoryState}
          onPassageBack={onPassageBack}
          onPassageForward={onPassageForward}
          isNarrowScreen={isNarrowScreen}
          setPassageHistoryState={setPassageHistoryState}
          isInternalNavigation={isInternalNavigation} // ← ПЕРЕДАЄМО
        />
...
const PassagePage = memo(({ lang }) => {
  const [panels, setPanels] = useState([
    {
      id: Date.now(),
      // key: `panel-${Date.now()}`, // Додаємо timestamp для примусового ререндеру
      key: `panel-${Date.now()}-0`,
      isNavigatingFromHistory: false, // ← ПОЧАТКОВО false
    },
  ]);
  ...
  useEffect(() => {
    // Ініціалізуємо історію при першому завантаженні
    const state = globalHistoryManager.getPassageState();

    // if (state.history.length === 0 && panels.length > 0) {
    //   const [book, chapter] = panels[0].initialRef?.split(".") || ["GEN", "1"];

    //   globalHistoryManager.addPassageEntry({
    //     ref: panels[0].initialRef || "GEN.1",
    //     versions: panels[0].initialVersions || ["LXX", "UTT"],
    //     book,
  ...
  const handlePassageBack = useCallback(() => {
    console.log("← Натиснуто назад в історії");
    const result = globalHistoryManager.goBackPassage();

    if (result && result.ref && result.versions) {
      console.log("Знайдено попередній запис:", result.ref, result.versions);

      // Оновлюємо ВСІ панелі
      setPanels((prev) =>
        prev.map((p, index) => ({
          ...p,
          initialRef: result.ref,
          initialVersions: [...result.versions],
          // Додаємо timestamp для примусового ререндеру
          // _timestamp: Date.now(),
          isNavigatingFromHistory: true, // ← ВАЖЛИВО!
          key: `panel-${Date.now()}-${index}`, // КЛЮЧ - найважливіше! React перестворить компонент при зміні key
        })),
      );

      setPassageHistoryState(result.state);
    }
  }, []);

  const handlePassageForward = useCallback(() => {
    console.log("→ Натиснуто вперед в історії");
    const result = globalHistoryManager.goForwardPassage();

    if (result && result.ref && result.versions) {
      console.log("Знайдено наступний запис:", result.ref, result.versions);
    ...
    <div className="passage-container">
      <div className="passage-panels">
        {panels.map((panel, index) => (
          <Panel
            // key={panel.id}
            // ВИКОРИСТОВУЄМО key, НЕ panelKey!
            key={panel.key || panel.id}
            // panelKey={`${panel.id}-${panel.initialRef || ""}-${Date.now()}`}
            ...
            // ──── нові пропси ────
            passageHistoryState={passageHistoryState}
            onPassageBack={handlePassageBack}
            onPassageForward={handlePassageForward}
            initialRef={panel.initialRef}
            initialVersions={panel.initialVersions}
            setPassageHistoryState={setPassageHistoryState} // ← ДОДАТИ
            // ... інші props
