...
// // src/components/InterlinearVerse.js - ОПТИМІЗОВАНИЙ 14.01.2026-2

// // src/components/InterlinearVerse.js - ВИПРАВЛЕНА ВЕРСІЯ
// import React, {
//   useState,
//   useRef,
//   useMemo,
...
// import "../styles/Interlinear.css";
// import { jsonAdapter, getValue } from "../utils/jsonAdapter";
// import { logger } from "../utils/logger"; // ПРАВИЛЬНИЙ ІМПОРТ

// const InterlinearVerse = memo(
//   ({ verseNum, pairs, chapterData, onWordClick, isFirstInChapter = false }) => {
//     const [hoveredWord, setHoveredWord] = useState(null);
//     const [mousePos, setMousePos] = useState({ x: 0, y: 0 });
//     const [isAboveCursor, setIsAboveCursor] = useState(false);
//     const tooltipRef = useRef(null);
...
//     const getWordText = useCallback((word) => {
//       return word
//         ? getValue(word, "word") || getValue(word, "w") || null
//         : null;
//     }, []);
...
//     const getLemma = useCallback((word) => {
//       return word
//         ? getValue(word, "lemma") || getValue(word, "l") || null
//         : null;
//     }, []);

//     const getMorph = useCallback((word) => {
//       return word
//         ? getValue(word, "morph") || getValue(word, "m") || null
//         : null;
//     }, []);
...
//               translationMap.set(strong, []);
//             }
//             translationMap.get(strong).push({ word: transWord, index: i });
//           }
//         }

//         // Обробка оригінальних слів
//         for (let i = 0; i < origWords.length; i++) {
//           const origWord = origWords[i];
//           const origStrong = getStrongCode(origWord, originalVersion);
...
//             if (candidates.length > 0) {
//               matchedWord = candidates[0].word;
//               // Видаляємо використаного кандидата
//               candidates.shift();
//               if (candidates.length === 0) {
//                 translationMap.delete(origStrong);
//               }
...
//         return aligned;
//       },
//       [getStrongCode, getWordsFromVerse]
//     );
...
//         // Створення блоків з оригіналу
//         for (let wordIndex = 0; wordIndex < origWords.length; wordIndex++) {
//           const origWord = origWords[wordIndex];
//           if (!blocks[wordIndex]) {
//             blocks[wordIndex] = {
//               id: `${verseNum}-${wordIndex}`,
...
//           for (
//             let transIndex = 0;
//             transIndex < pair.translations.length;
//             transIndex++
//           ) {
//             const transCode = pair.translations[transIndex];
//             const translationKey = `${pair.original}-${transCode}`;
...
//       const endTime = performance.now();
//       logger.debug(
//         `Створення блоків для вірша ${verseNum}: ${(
//           endTime - startTime
//         ).toFixed(2)}мс`
//       );
...
// // Додамо displayName для кращої ідентифікації в DevTools
// InterlinearVerse.displayName = "InterlinearVerse";
