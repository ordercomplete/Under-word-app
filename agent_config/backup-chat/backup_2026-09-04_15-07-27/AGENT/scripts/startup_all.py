...
# Python scripts to load
CORE_SCRIPTS = {
    "anti_loop": HOOKS_DIR / "anti_loop.py",
    "watch_agent_file": HOOKS_DIR / "watch_agent_file.py",
    "str_translate": HOOKS_DIR / "str.translate.py",
}
...
def watcher_is_running() -> bool:
    ...
    result = subprocess.run(
        ["pgrep", "-f", str(HOOKS_DIR / "watch_agent_file.py")],
        capture_output=True,
        text=True,
        check=False,
    )
...
def start_watcher() -> None:
    ...
    proc = subprocess.Popen(
        [sys.executable, str(HOOKS_DIR / "watch_agent_file.py")],
        cwd=str(WORKSPACE),
